...
/**
 * Physical units ("Exemplare") one ITEM record may list individually. A unit is
 * in exactly one of three derived states, never stored: FREE (on the shelf),
 * OUT (named on an open checkout line) or OOS (flagged out of service). The cap
 * is far below TRAX_MAX_QUANTITY on purpose — units are hand-managed rows an
 * operator reads, not a bulk counter.
 */
if (!defined('TRAX_MAX_UNITS')) define('TRAX_MAX_UNITS', 500);
 ...
 * case-insensitive (RFC 3986 §3.1, §3.2.2) and the path is digits with an
 * optional ".unit" suffix ("/12.1"), so the upper-case URL is the same
 * resource.
 */
function trax_label_url(int $id, ?int $unit = null): string
{
    $base = '';
    ...
    if (TRAX_LABEL_URL_FORM === 'short' && !preg_match('/[a-z]/', TRAX_LABEL_SHORT_PATH)) {
        ...
        // A unit is addressed as "12.1". The dot is in QR's alphanumeric
        // charset (unlike '?' and '='), so the short form stays small even
        // once it names a single unit.
        $suffix = $unit !== null && $unit >= 1 ? '.' . $unit : '';

        return strtoupper($base . $path . $id . $suffix);
    }

    return $base . TRAX_PUBLIC_PATH . '?id=' . $id
        . ($unit !== null && $unit >= 1 ? '&u=' . $unit : '');
}
