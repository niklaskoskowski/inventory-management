...
function trax_item_pair(mixed $raw, int $maxQty = TRAX_MAX_QUANTITY): array
{
    ...
    return [$id, max(1, min($maxQty, $qty))];
}
...
{
    $raw = is_array($raw) ? $raw : [];
    [$assetId, $qty] = trax_item_pair($raw);
...
/** Units of an asset that could go out right now. */
function trax_available_qty(array $asset, array $checkouts): int
{
    $quantity = max(1, (int)($asset['quantity'] ?? 1));
    return max(0, $quantity - trax_out_qty((int)$asset['id'], $checkouts));
}
...
function trax_effective_status(array $asset, array $assetsById, array $linesByAssetId): string
{
    ...
    if ($asset['kind'] !== 'SET') {
        $quantity  = max(1, (int)($asset['quantity'] ?? 1));
        $available = max(0, $quantity - trax_lines_qty($linesByAssetId[$asset['id']] ?? []));
        if ($available === 0) {
            return 'UNAV';
        }
    ...
    foreach ($asset['members'] as $member) {
        ...
        $quantity  = max(1, (int)($target['quantity'] ?? 1));
        $available = max(0, $quantity - trax_lines_qty($linesByAssetId[$memberId] ?? []));
 ...
 *
 * @param  array $items  bare ids and/or {assetId|id, qty}
 * @return array<int, array{assetId:int, qty:int}>
 */
function trax_expand_items(array $items, array $assetsById): array
{
    ...
    $add = static function (int $assetId, int $qty) use (&$out, &$index): void {
        $qty = max(1, min(TRAX_MAX_QUANTITY, $qty));
        ...
        $out[]           = ['assetId' => $assetId, 'qty' => $qty];
    };

    foreach ($items as $item) {
        [$id, $qty] = trax_item_pair($item);
