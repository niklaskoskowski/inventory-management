...
import {
  ...
  selectedUnitCount, mutate, toast,
} from '../store.js';
...
export default {
  ...
  setup(props, { emit }) {
     ...
     *     field, so this must never stop working
     *   - the short URL, HTTPS://HOST/3, which is upper case because that is
     *     what puts it in QR's alphanumeric mode (21 modules instead of 25)
     *
     * Case-insensitive: URL parsing already lower-cases scheme and host, and
     * the loose fallbacks below carry the /i themselves.
     */
    const extractId = (text) => {
      ...
      try {
        ...
        // Short form: the whole path is the id, e.g. /3 or /R/3.
        const tail = url.pathname.match(/\/(\d+)\/?$/);
      ...
      const path = raw.match(/^[a-z][a-z0-9+.-]*:\/\/[^/?#]+\/(?:[^/?#]+\/)*(\d+)\/?$/i);
    ...
    const handle = async (id) => {
      ...
      hits.value.unshift({ at: Date.now(), asset });
    ...
    const submitManual = () => {
      const id = extractId(manualId.value);
      if (id === null) {
        toast('Enter a numeric asset ID.', 'warning');
        return;
      }
      manualId.value = '';
      handle(id);
    ...
    return {
      ...
      REGION_ID,
      cameraFix,
      modeHint, switchCamera, submitManual, start, stop, restart, onTapStart, extractId,
